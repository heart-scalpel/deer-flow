...
def main():
    ...
    while True:
        try:
            ...
            sid_short = engine.current_session_id[:8]
            agent_name = getattr(engine.client, '_agent_name', None) or "lead-agent"
            prompt = f"[{sid_short}|{agent_name}] You: "
            ...
            if user_input.lower() == '!multi':
                sid_short = engine.current_session_id[:8]
                agent_name = getattr(engine.client, '_agent_name', None) or "lead-agent"
                user_input = multi_line_input(f"\n[{sid_short}|{agent_name}] Multi-line Input Mode | 多行输入模式")
            ...
            if user_input.lower() == "!help":
                ...
                print("    !enable <pattern>  Enable skill(s) - supports regex | 启用技能（支持正则）")
                print("    !disable <pattern> Disable skill(s) - supports regex | 禁用技能（支持正则）")
                print("  Agents | 自定义 Agent:")
                print("    !agents            List custom agents | 列出自定义 agent")
                print("    !create_agent <name> [desc] Register a new agent | 注册新 agent（不写 SOUL.md）")
                print("    !agent [name]      Show or switch current agent | 显示或切换当前 agent")
                print("    !agent_skills [name] Show agent's skill whitelist | 查看 agent 的 skill 白名单")
                print("    !bind_skills <agent> <skill...> Bind skills to agent | 为 agent 绑定 skill（--clear 清空）")
            ...
            if user_input.lower().startswith("!enable"):
                parts = user_input.split()
                if len(parts) < 2:
                    print("[Error] Usage: !enable <skill_name|pattern> | 用法: !enable <技能名|正则>")
                    continue
            ...
            if user_input.lower().startswith("!disable"):
                parts = user_input.split()
                if len(parts) < 2:
                    print("[Error] Usage: !disable <skill_name|pattern> | 用法: !disable <技能名|正则>")
                    continue
            ...
            if user_input.lower().startswith("!create_agent"):
                parts = user_input.split(maxsplit=2)
                if len(parts) < 2:
                    print("[Error] Usage: !create_agent <name> [description] | 用法: !create_agent <名字> [描述]")
                    continue
                agent_name = parts[1]
                description = parts[2] if len(parts) > 2 else ""
                engine.create_agent(agent_name, description=description)
                continue

            # NOTE: !agent_skills must be checked before !agent to avoid prefix collision.
            if user_input.lower().startswith("!agent_skills"):
                parts = user_input.split()
                engine.get_agent_skills(parts[1] if len(parts) > 1 else None)
                continue

            if user_input.lower().startswith("!bind_skills"):
                parts = user_input.split()
                if len(parts) < 2:
                    print("[Error] Usage: !bind_skills <agent> <skill...> | !bind_skills <agent> --clear | 用法见 !help")
                    continue
                agent = parts[1]
                rest = parts[2:]
                if not rest:
                    print("[Error] Provide at least one skill name, or use --clear. | 至少一个 skill，或用 --clear")
                    continue
                if rest == ["--clear"]:
                    engine.bind_agent_skills(agent, None)
                else:
                    engine.bind_agent_skills(agent, rest)
                continue

            if user_input.lower().startswith("!agent"):
                parts = user_input.split()
                if len(parts) == 1:
                    current = getattr(engine.client, '_agent_name', None) or "lead-agent"
                    print(f"\n[Agent] Current: {current}\n")
                    continue
                engine.switch_agent(parts[1])
                continue
