...
class SubagentExecutor:
    ...
    def __init__(
        ...
        Args:
            ...
            user_id: User ID for tracing. When None, Langfuse uses DEFAULT_USER_ID.
        """
        self.config = config
        ...
        self.user_id = user_id  # Capture user_id from parent context for tracing
    ...
    async def _aexecute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            if tracing_callbacks:
                existing_callbacks = run_config.get("callbacks") or []
                run_config["callbacks"] = [*existing_callbacks, *tracing_callbacks]

            # Normalize subagent name for tracing (mirrors build_run_config logic)
            # to ensure consistency with lead agent naming: lowercase, hyphens only.
            if self.config.name:
            ...
            inject_langfuse_metadata(
                ...
                user_id=self.user_id,  # Passed from parent, or None (will use DEFAULT_USER_ID)
